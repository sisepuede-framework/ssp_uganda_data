...
combined <- subset(combined, !(Subsector_Category %in% sector_switch))
...
hist <- combined$value[combined$strategy == "Historical" &
                         combined$Subsector_Category %in% sector_switch &
                         combined$Year == 2022]
...
combined$value[combined$strategy == "Historical" & 
                 combined$Subsector_Category %in% sector_switch ] <- combined$value[combined$strategy == "Historical" &
                                                                               combined$Subsector_Category %in% sector_switch ] * diff
combined$value[combined$strategy == "Historical" &
                 combined$Subsector_Category %in% sector_switch &
                 combined$Year == 2022]


# Replace sector for baseline 22
df <- subset(combined, !(strategy_id == 6008 & 
                     CSC.Subsector == "Buildings" & 
                     Year >= 2023))

df_22 <- read.csv(paste0(dir.tableau,'raw_emissions_uganda_2022_WIDE_INPUTS_OUTPUTS.csv'))
df_22 <- subset(df_22, strategy_id==6008 & CSC.Subsector=='Buildings' )

combined <- rbind(df, df_22)

combined <- combined[!duplicated(combined[, c("strategy_id", "CSC.Subsector", "Year", "Gas", "primary_id")]), ]

subset(combined, strategy_id==6008 & CSC.Subsector=='Buildings' & Year==2022 )
