...
hp_filter_subsec <- function(data,
  ...
  # Apply HP filter only if strategy_id is not NA
  dt[`CSC.Subsector` == subsec_target & Gas %in% gas_target & !is.na(strategy_id),
     value_hp := {
       ...
       sm <- pmax(hp$trend, 0)
       out <- numeric(.N); out[o] <- sm; out
     },
     by = by_cols
  ]
  
  # Prepare plot data (keep also historical NA strategy_id)
  ...
  p <- ggplot(plot_dt, aes(x = .data[[time_col]])) +
    ...
    geom_line(aes(y = value_hp, colour = "HP Filter"), size = 1, na.rm = TRUE) +
    scale_colour_manual(values = c("Original" = "steelblue", "HP Filter" = "red")) +
    labs(
      x = time_col,
      y = value_col,
      title = paste("HP Filter (lambda =", lambda_hp, ") -", subsec_target, "-", gas_target),
      colour = "Series"
    ) +
    facet_wrap(~strategy_id, scales = facet_scale) +
    theme_minimal()
  
  # Replace original values only after plotting
  if (replace_original) {
    dt[`CSC.Subsector` == subsec_target & Gas %in% gas_target & !is.na(strategy_id) & !is.na(value_hp),
       (value_col) := value_hp]
  }
  
  return(list(data = dt, plot = p))
}

# HP filter for AG - Livestock
...
res3 <- hp_filter_subsec(
  data = res2$data,
  subsec_target = "IN - Industrial Processes",
  gas_target = "CO2",
  lambda_hp = 100
)
...
write.csv(res4$data,paste0(dir.tableau,file.name),row.names=FALSE)
