...
process_year <- function(y) {
  ...
  # leer con fallback si cambia el rango
  df <- tryCatch(
    read_excel(path, sheet = "Table A Summary Table", range = "A3:D113"),
    error = function(e) read_excel(path, sheet = "Table A Summary Table")
  )
  
  names(df)[1] <- "Categories"
...
panel <- map_dfr(years, ~{
  message("Procesando: ", .x)
  tryCatch(process_year(.x), error = function(e) {warning(paste("Saltando", .x, ":", e$message)); NULL})
})
